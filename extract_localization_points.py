...
def parse_log(log_path):
    """将一个 replica_conductance.log 转换为一个拟合数据点。"""
    ...
    mean_ln_g = float(extract_number(text, "mean ln(G/G0)"))
    std_ln_g = float(extract_number(text, "std ln(G/G0)"))
    ...
    sem_ln_g = std_ln_g / math.sqrt(n_valid)
    ...
    return {
        ...
        "mean_ln_G_over_G0": mean_ln_g,
        "std_ln_G_over_G0": std_ln_g,
        "sem_ln_G_over_G0": sem_ln_g,
...
def collect_points(root, temperatures=None, structure=None):
    """递归查找日志，并按筛选条件返回拟合点。"""
    points = []
    failures = []
    ...
    points.sort(key=lambda p: (p["temperature_K"], p["length_A"]))
...
def plot_points(points, output_path):
    ...
    for index, temperature in enumerate(temperatures):
        group = [
            p for p in points
            if math.isclose(p["temperature_K"], temperature, abs_tol=1e-8)
        ]
